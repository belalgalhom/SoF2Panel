    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 0.5rem;">
        ...
        <div style="display: flex; align-items: center; gap: 1rem;">
            ...
            <form action="{{ route('tickets.close', $ticket) }}" method="POST" style="margin: 0;">
                @csrf
                <button class="btn btn-danger" style="width: auto;">Close Ticket</button>
            </form>
            @endif
        </div>
    </div>
...
{{-- Chat Messages --}}
<div class="glass-panel" style="max-width: none; padding: 1.5rem; margin-bottom: 1.5rem; display: flex; flex-direction: column; gap: 1.5rem;">
    @forelse($ticket->messages as $msg)
        ...
        <div style="display: flex; gap: 1rem; flex-direction: {{ $isOwn ? 'row-reverse' : 'row' }};">
            ...
            <div style="max-width: 70%; display: flex; flex-direction: column; align-items: {{ $isOwn ? 'flex-end' : 'flex-start' }};">
                ...
                <div style="padding: 0.85rem 1.1rem; border-radius: 14px; font-size: 0.92rem; line-height: 1.6; white-space: pre-wrap; word-break: break-word;
                    {{ $isOwn
